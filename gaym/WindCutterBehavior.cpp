...
void WindCutterBehavior::OnChannelTick(GameObject* caster, const DirectX::XMFLOAT3& target, float tickMult)
{
    ...
    if (m_pVFXManager && EffectRegistry::Get().HasEffect("sub_wind"))
    {
        XMFLOAT3 spawnPos = { origin.x, origin.y + 0.5f, origin.z };
        m_pVFXManager->SpawnEffectDef(spawnPos, dir,
            EffectRegistry::Get().GetEffect("sub_wind"), false);
    }
