...
void EndingScreen::Update(InputSystem& input, float screenW, float screenH, float dt)
{
    if (m_result != Result::None) return;

    float fNext = m_fFade + dt * 1.0f;
    m_fFade = (fNext > 1.0f) ? 1.0f : fNext;
    if (m_fInputCD > 0.f) { m_fInputCD -= dt; return; }

    const float btnW = 320.f, btnH = 96.f, gap = 24.f;
    float cx = screenW * 0.5f;
    float by = screenH * 0.66f;
    m_btnTitle = { cx - btnW * 0.5f, by,                btnW, btnH };
    ...
    m_nHoverBtn = hit(m_btnTitle) ? 0 : (hit(m_btnQuit) ? 1 : -1);

    if (input.IsMouseButtonPressed(0)) {
        if      (m_nHoverBtn == 0) m_result = Result::ToTitle;
...
void EndingScreen::Render(SpriteBatch* pBatch, SpriteFont* pFont,
                           float screenW, float screenH)
{
    ...
    if (m_hTitle.ptr) {
        ...
        XMFLOAT2 pos((screenW - w) * 0.5f, screenH * 0.18f);
    ...
    drawBtn(m_btnTitle, L"TITLE", m_nHoverBtn == 0);
